...
RUN git clone https://github.com/qiita-spots/qp-deblur.git
WORKDIR qp-deblur
RUN pip install -e .
...
RUN /qp-deblur/scripts/configure_deblur --env-script "true" --server-cert `find /qiita_server_certificates/ -name "*_server.crt" -type f`
