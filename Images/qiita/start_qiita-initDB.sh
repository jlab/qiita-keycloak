...
# create (empty) workding directory as given in Qiita configuration
mkdir -p `grep ^WORKING_DIR $QIITA_CONFIG_FP | cut -d "=" -f 2`

sleep 5 # wait for postgress to boot up
...
source $CONDA_DIR/etc/profile.d/conda.sh; conda activate $CONDA_DIR/envs/$ENV_NAME; python /drop_workflows.py; python /secure_db.py
