...
.built_image_qtp-multiqc-report: Images/qtp-multiqc-report/qtp-multiqc-report.dockerfile Images/qtp-multiqc-report/start_qtp-multiqc-report.sh src/qtp-multiqc-report Certificates/
	tmpdir=$(TMPDIR) $(MAKE) plugin
	cp -r $^ $(TMPDIR)
	$(PODMAN_BIN) build $(TMPDIR)/ -f $(TMPDIR)/`basename $<` $(PODMAN_FLAGS) -t local-`basename $< | cut -d "." -f 1`
	touch .built_image_`basename $< | cut -d "." -f 1`
